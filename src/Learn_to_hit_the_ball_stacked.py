...
if __name__ == '__main__':
    ...
    if load_filename is not None:
        agent = PPO.load(load_filename, env, verbose=1, n_steps=horizon)
    else:
        agent = PPO("MultiInputPolicy", env, verbose=1, n_steps=horizon)
    print(agent.policy)
    ...
    callbacks = [savemod, rew, vid, varsched, reward_record]
